...
//                .isThrownBy(() -> toTest.createGroup(LocalDateTime.now(), 0));
    ...
    void createGroup_configuration_existingProject_createsAndSavesGroup() {
        ...
        TaskConfigurationProperties mockConfig = configurationReturning(true);
        //system under test
        var toTest = new ProjectService(mockRepository,
                inMemoryGroupRepo, serviceWithInMemRepo, mockConfig);

        //when
        GroupReadModel result = toTest.createGroup(toDay, 1);
        //then
        assertThat(result.getDescription()).isEqualTo("bar");
//                .hasFieldOrProperty("description","bar");
        assertThat(result.getDeadline()).isEqualTo(toDay.minusDays(1));
        assertThat(result.getTasks()).allMatch(task -> task
                .getDescription().equals("foo"));
